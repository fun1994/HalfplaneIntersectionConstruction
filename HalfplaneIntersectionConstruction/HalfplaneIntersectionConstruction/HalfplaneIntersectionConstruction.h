...
class HalfplaneIntersectionConstruction {
	...
	bool isBounded(std::vector<Point>& V, int& index) {
		...
		for (int i = 0; i < V.size(); i++) {
			if (V[i].index.second != V[(i + 1) % V.size()].index.first) {
				index = (i + 1) % V.size();
				return false;
			}
		}
...
public:
	std::vector<Halfplane> bruteForce(std::vector<Halfplane>& H) {
		if (H.size() == 1) {
			return H;
		}
		std::vector<Halfplane> C;
		std::vector<Point> V;
		for (int i = 0; i < H.size(); i++) {
			for (int j = i + 1; j < H.size(); j++) {
				Point p = intersectionPoint(H[i], H[j]);
				if (inConvexPolygon(H, p, i, j)) {
					if (crossProduct(H[i], H[j]) > 0) {
						p.index.first = i;
						p.index.second = j;
					}
					else {
		...
		for (int i = 0; i < V.size(); i++) {
			C.push_back(H[V[i].index.first]);
		}
		if (!flag) {
			C.push_back(H[V[V.size() - 1].index.second]);
		}
		return C;
	}
	std::vector<Halfplane> divideAndConquer(std::vector<Halfplane>& H) {
		if (H.size() == 1) {
			return H;
		}
		int mid = H.size() / 2;
		std::vector<Halfplane> H1(H.begin(), H.begin() + mid);
		std::vector<Halfplane> H2(H.begin() + mid, H.end());
		std::vector<Halfplane> C1 = divideAndConquer(H1);
		std::vector<Halfplane> C2 = divideAndConquer(H2);
		std::vector<Halfplane> C = planeSweeping(C1, C2);
		return C;
	}
};
